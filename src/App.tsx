...
function App() {
  ...
  // Accounts shared state for high-fidelity interactivity
  ...
  const [dataEntries, setDataEntries] = useState<DataEntry[]>([
    {
      ...
      deskripsi: 'Masyarakat adat Kasepuhan Ciptagelar memasukkan varietas benih lokal padi 12 ke database untuk melestarikan keanekaragaman crop wild relatives.',
      images: ['https://images.unsplash.com/photo-1530595467537-0b5996c41f2d?auto=format&fit=crop&q=80&w=600&h=450'],
    ...
    },
    {
      ...
      deskripsi: 'Fasilitator lapangan menyelesaikan dokumen pendaftaran data sebaran dan status konservasi Desa Sukamaju.',
      images: ['https://images.unsplash.com/photo-1464226184884-fa280b87c399?auto=format&fit=crop&q=80&w=600&h=450'],
  ...
  // Toggle Account Active Status Callback
  ...
  // Toggle Entry status between Aktif and Verifikasi
  const handleToggleEntryStatus = (id: string): void => {
    setDataEntries((prev) =>
      prev.map((entry) =>
        entry.id === id
          ? { ...entry, status: entry.status === 'Aktif' ? 'Verifikasi' : 'Aktif' }
          : entry
      )
    );
  };
  ...
  // Get active editing account
  const currentEditingAccount = accounts.find((a) => a.id === editingAccountId) || accounts[0];

  // Get active validating data entry
  const currentActiveEntry = dataEntries.find((e) => e.id === activeEntryId) || dataEntries[0];
            ...
            <div className="flex flex-col space-y-1.5 max-h-72 overflow-y-auto pr-1">
              {[
                ...
                <button
                  key={pg.id}
                  onClick={() => {
                    if (pg.id === 'edit-account' && accounts.length > 0 && !editingAccountId) {
                      setEditingAccountId(accounts[0].id);
                    }
                    if (pg.id === 'validasi-data' && dataEntries.length > 0 && !activeEntryId) {
