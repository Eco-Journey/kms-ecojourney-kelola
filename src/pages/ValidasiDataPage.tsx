...
interface ValidasiDataPageProps {
  ...
  onValidateEntry: (id: string, newStatus: 'Aktif' | 'Verifikasi' | 'Ditolak' | string) => void;
}

export default function ValidasiDataPage({ entry, onNavigate, onValidateEntry }: ValidasiDataPageProps): React.ReactElement {
  ...
  const currentEntry: DataEntry = entry || {
    ...
    lokasi: {
      kota: 'Surabaya',
      provinsi: 'Surabaya',
      deskripsiLokasi: 'Terdapat di Surabaya bagian Timur, Desa ABCD, Kecamatan XYZ, deket rumah Pak Mamat',
      koordinat: { lat: -7.2575, lng: 112.7521 }
    },
    deskripsi: 'Varietas ini merupakan Padi yang tumbuh dengan ekosistem lorem ipsum dolor sit amet.',
    images: [
  ...
  const handleApprove = (): void => {
    ...
    alert('Data berhasil divalidasi dan dipublikasikan!');
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-7 space-y-6">
              ...
              {currentEntry.type === 'Pengetahuan Adat' ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-5">
                  <div className="sm:col-span-2">
                    <span className="text-[10px] font-bold text-gray-500 uppercase block tracking-wider mb-1">Judul Pengetahuan Adat</span>
                    <span className="text-sm font-semibold text-gray-950 block">{currentEntry.nama}</span>
                  </div>
                  <div>
                    <span className="text-[10px] font-bold text-gray-500 uppercase block tracking-wider mb-1">Varietas Terkait</span>
                    <span className="text-sm font-normal text-gray-700 block">{currentEntry.varietasTerkait || '-'}</span>
                  </div>
                  <div>
                    <span className="text-[10px] font-bold text-gray-500 uppercase block tracking-wider mb-1">Wilayah Asal</span>
                    <span className="text-sm font-normal text-gray-700 block">{currentEntry.wilayahAsal || '-'}</span>
                  </div>
                  <div>
                    <span className="text-[10px] font-bold text-gray-500 uppercase block tracking-wider mb-1">Kategori</span>
                    <span className="text-sm font-normal text-gray-700 block">{currentEntry.kategori}</span>
                  </div>
                  <div>
                    <span className="text-[10px] font-bold text-gray-500 uppercase block tracking-wider mb-1">Tanggal Publikasi</span>
                    <span className="text-sm font-normal text-gray-700 block">{currentEntry.tanggal}</span>
                  </div>
                </div>
              ) : (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-5">
                  <div className="sm:col-span-2">
                    <span className="text-[10px] font-bold text-gray-500 uppercase block tracking-wider mb-1">Nama Varietas/Temuan</span>
                    <span className="text-sm font-semibold text-gray-950 block">{currentEntry.nama}</span>
                  </div>
                  <div>
                    <span className="text-[10px] font-bold text-gray-500 uppercase block tracking-wider mb-1">Nama Lokal</span>
                    <span className="text-sm font-normal text-gray-700 block">{currentEntry.namaLokal || '-'}</span>
                  </div>
                  <div>
                    <span className="text-[10px] font-bold text-gray-500 uppercase block tracking-wider mb-1">Nama Ilmiah</span>
                    <span className="text-sm font-normal text-gray-700 block italic">{currentEntry.namaIlmiah || '-'}</span>
                  </div>
                  <div>
                    <span className="text-[10px] font-bold text-gray-500 uppercase block tracking-wider mb-1">Kategori</span>
                    <span className="text-sm font-normal text-gray-700 block">{currentEntry.kategori}</span>
                  ...
                  <div>
                    <span className="text-[10px] font-bold text-gray-500 uppercase block tracking-wider mb-1">Tanggal Penemuan</span>
                    <span className="text-sm font-normal text-gray-700 block">{currentEntry.tanggal}</span>
                  </div>
                </div>
              )}
            ...
            <div className="bg-white rounded-[5px] border border-gray-200/50 shadow-sm p-6 md:p-8 space-y-6">
              <div>
                <h2 className="text-2xl font-extrabold text-gray-900 tracking-tight">
                  {currentEntry.type === 'Pengetahuan Adat' ? 'Detail Lokasi dan Narasumber' : 'Detail Lokasi dan Penemu'}
                </h2>
                <hr className="border-gray-200 mt-4" />
              </div>
              ...
              <div className="space-y-5">
                <div>
                  <span className="text-[10px] font-bold text-gray-500 uppercase block tracking-wider mb-1">
                    {currentEntry.type === 'Pengetahuan Adat' ? 'Nama Narasumber' : 'Nama Penemu'}
                  </span>
                  <span className="text-sm font-semibold text-gray-950 block">
                    {currentEntry.type === 'Pengetahuan Adat' ? currentEntry.namaNarasumber : currentEntry.namaPenemu}
                  </span>
                </div>

                {/* Map Display (readOnly mode) */}
                <div>
                  <span className="text-[10px] font-bold text-gray-500 uppercase block tracking-wider mb-1">Koordinat</span>
                  <MapLocator 
                    readOnly={true} 
                    latitude={currentEntry.lokasi.koordinat ? currentEntry.lokasi.koordinat.lat : null} 
                    longitude={currentEntry.lokasi.koordinat ? currentEntry.lokasi.koordinat.lng : null} 
                  />
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <span className="text-[10px] font-bold text-gray-500 uppercase block tracking-wider mb-1">Kota/Kabupaten</span>
                    <span className="text-sm font-normal text-gray-700 block">{currentEntry.lokasi.kota}</span>
                  </div>
                  <div>
                    <span className="text-[10px] font-bold text-gray-500 uppercase block tracking-wider mb-1">Provinsi</span>
                    <span className="text-sm font-normal text-gray-700 block">{currentEntry.lokasi.provinsi}</span>
                  </div>
                </div>

                <div>
                  <span className="text-[10px] font-bold text-gray-500 uppercase block tracking-wider mb-1">Deskripsi Lokasi</span>
                ...
                <div>
                  <span className="text-[10px] font-bold text-gray-500 uppercase block tracking-wider mb-1">Deskripsi</span>
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 pt-6 border-t border-gray-150">
                <div className="flex items-center space-x-2">
                  <span className="text-xs font-bold text-gray-600">Status Pengajuan:</span>
          ...
          <div className="lg:col-span-5 space-y-6">
            ...
            <div className="bg-white rounded-[5px] border border-gray-200/50 shadow-sm p-6 space-y-4">
                    ...
                    {currentEntry.images.map((img, idx) => (
                      <div 
                        ...
                        className={`w-12 h-12 rounded overflow-hidden aspect-square border-2 cursor-pointer ${
                          idx === activeImageIndex ? 'border-kms-green-dark scale-105' : 'border-gray-200'
                        }`}
              ...
              <div className="flex space-x-2 pt-2">
                <button
                  type="button"
                  onClick={() => alert('Fitur edit gambar hanya aktif pada form pengajuan.')}
                ...
                <button
                  type="button"
                  onClick={() => alert('Foto tidak dapat dihapus dalam mode validasi ini.')}
            ...
            <div className="bg-white rounded-[5px] border border-gray-200/50 shadow-sm p-6 space-y-4">
              ...
              <div className="border border-gray-200 bg-slate-50 rounded-[5px] p-4 flex items-center justify-between">
                <div className="flex items-center space-x-3 text-kms-green-dark">
                  ...
                  <div className="text-left">
                    <p className="text-xs font-bold truncate max-w-xs">{currentEntry.fpicDoc}</p>
                    <p className="text-[10px] text-gray-400">PDF Document • Signed</p>
                  </div>
                </div>
                
                <button
                  type="button"
                  onClick={() => alert(`Mengunduh dokumen: ${currentEntry.fpicDoc}`)}
